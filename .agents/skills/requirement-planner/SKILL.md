...
5. Based on your understanding and research, create a detailed plan outlining the steps to implement the requirements, including any assumptions made and potential risks identified. There is exactly one canonical plan file in `updated-plan/`. Rewrite it in place when the project state changes. Do NOT keep historical copies in the working tree — git history preserves the evolution if anyone wants to see it. Old `plan-<date>.md` files in this folder are noise; delete them. The filename pattern stays `plan-<dd-mm-yyyy>.md` and you may rename to the new date when you do a major rewrite.
